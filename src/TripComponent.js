...
const getCurrentCoordinates = number =>
  trip.trackPoints
    .filter(item => item.class === number)
    .map(item => {
      return {latitude: item.lat, longitude: item.lon};
    });

const class1 = [...getCurrentCoordinates(1)];
const class2 = [...getCurrentCoordinates(2)];
const class3 = [...getCurrentCoordinates(3)];
const class4 = [...getCurrentCoordinates(4)];
const class5 = [...getCurrentCoordinates(5)];
